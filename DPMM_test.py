...
clusters = [Gaussian(np.asarray([]), prior) for _ in range(cluster_count + 1)]
...
model = DPMM(cluster_count, data_count, alpha, clusters, data, membership)

CollapsedGibbsSampling(model, 1)